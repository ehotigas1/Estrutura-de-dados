...
int esta_cheia(Lista *lista) {
    return lista->n == TAMANHO; // retorna 1 se numero de elementos (n) é igual ao tamanho da lista 
}
...
int esta_vazia(Lista *lista) {
    return lista->n == 0; //retorna 1 se (n) igual a zero
}
...
int encontrar_posicao(Lista *lista, int valor) {
    int idx = 0;
    while(idx < lista->n && valor > lista->valores[idx]){ //explicado no tablet lab atividade 1
        idx++;
    }
    return idx;
}
...
void deslocar_direita(Lista *lista, int indice) {
    ...
    for(i = lista->n; i > indice; i--){
        lista->valores[i] = lista->valores[i - 1]; //representado no tablet (desenho em lapis roxo)
    }
}
